    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-800">Modifier : {{ $partner->name['es'] ?? 'Partenaire' }}</h2>
            </div>
            ...
            <form action="{{ route('admin.partners.update', $partner) }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <select name="type" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                            @foreach(App\Models\Partner::TYPES as $key => $label)
                                <option value="{{ $key }}" {{ old('type', $partner->type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="website" value="{{ old('website', $partner->website) }}" placeholder="https://..." class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>

                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Ordre</label>
                        <input type="number" name="order" value="{{ old('order', $partner->order) }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>

                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Statut</label>
                         <label class="flex items-center mt-2">
                            <input type="checkbox" name="active" value="1" {{ old('active', $partner->active) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-teal-600">
                ...
                <div class="mb-6">
                    <label class="block text-gray-700 font-bold mb-2">Logo</label>
                     @if($partner->logo)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $partner->logo) }}" alt="Logo actuel" class="h-20 object-contain">
                        </div>
                    @endif
                ...
                <div id="content-{{ $lang }}" class="{{ $lang === 'es' ? '' : 'hidden' }} lang-content">
                    <div class="mb-4">
                        ...
                        <input type="text" name="name[{{ $lang }}]" value="{{ old('name.'.$lang, $partner->name[$lang] ?? '') }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>
                    
                    <div class="mb-4">
                        <label class="block text-gray-700 font-bold mb-2">Description ({{ strtoupper($lang) }})</label>
                        <textarea name="description[{{ $lang }}]" rows="5" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('description.'.$lang, $partner->description[$lang] ?? '') }}</textarea>
                    </div>
                </div>
                @endforeach
