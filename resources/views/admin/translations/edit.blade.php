    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-800">Modifier : {{ $translation->group }}.{{ $translation->key }}</h2>
            </div>
            
            <form action="{{ route('admin.translations.update', $translation) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                
                <div class="mb-6">
                    <label class="block text-gray-700 font-bold mb-2">Notes (Contexte)</label>
                    <textarea name="notes" rows="2" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('notes', $translation->notes) }}</textarea>
                </div>
                ...
                <div id="content-{{ $lang }}" class="{{ $lang === 'es' ? '' : 'hidden' }} lang-content">
                    <div class="mb-4">
                        ...
                        <textarea name="text[{{ $lang }}]" rows="5" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('text.'.$lang, $translation->text[$lang] ?? '') }}</textarea>
                    </div>
                </div>
                @endforeach
