...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="mb-6">
            <a href="{{ route('admin.translations.index') }}" class="text-gray-500 hover:text-gray-700">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <form action="{{ route('admin.translations.store') }}" method="POST" class="p-6">
                @csrf
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Groupe</label>
                        <input type="text" name="group" value="{{ old('group') }}" placeholder="ex: header, auth, validation" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>

                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Clé</label>
                        <input type="text" name="key" value="{{ old('key') }}" placeholder="ex: welcome_message" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>
                </div>

                <div class="mb-6">
                    <label class="block text-gray-700 font-bold mb-2">Notes (Contexte)</label>
                    <textarea name="notes" rows="2" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('notes') }}</textarea>
                </div>
                ...
                <div id="content-{{ $lang }}" class="{{ $lang === 'es' ? '' : 'hidden' }} lang-content">
                    <div class="mb-4">
                        <label class="block text-gray-700 font-bold mb-2">Traduction ({{ strtoupper($lang) }})</label>
                        <textarea name="text[{{ $lang }}]" rows="5" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('text.'.$lang) }}</textarea>
                    </div>
                </div>
                @endforeach
