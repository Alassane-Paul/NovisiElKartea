            ...
            <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" value="{{ old('slug', $project->slug) }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-blue-500">
                ...
                <div id="content-{{ $lang }}" class="{{ $lang === 'es' ? '' : 'hidden' }} lang-content">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description[{{ $lang }}]" rows="3" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('description.'.$lang, $project->description[$lang] ?? '') }}</textarea>
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700 font-bold mb-2">Contenu ({{ strtoupper($lang) }})</label>
                        <textarea name="content[{{ $lang }}]" rows="10" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('content.'.$lang, $project->content[$lang] ?? '') }}</textarea>
                    </div>
                </div>
                @endforeach
