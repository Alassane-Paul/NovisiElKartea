...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-xl font-bold text-gray-800">Modifier : {{ $project->title['es'] ?? 'Projet' }}</h2>
        </div>
        ...
        <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="slug" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-blue-500">
                        <option value="afrikarte" {{ old('slug', $project->slug) == 'afrikarte' ? 'selected' : '' }}>Afrikarte</option>
                        <option value="diversidad" {{ old('slug', $project->slug) == 'diversidad' ? 'selected' : '' }}>Diversidad</option>
                        <option value="igualdad" {{ old('slug', $project->slug) == 'igualdad' ? 'selected' : '' }}>Igualdad</option>
                        <option value="new-generation" {{ old('slug', $project->slug) == 'new-generation' ? 'selected' : '' }}>New Generation</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="category" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-blue-500">
                        ...
                        <option value="culture" {{ old('category', $project->category) == 'culture' ? 'selected' : '' }}>Culture</option>
                        <option value="social" {{ old('category', $project->category) == 'social' ? 'selected' : '' }}>Social</option>
                    </select>
                </div>
                ...
                <div>
                    <label class="flex items-center mt-8">
                        <input type="checkbox" name="featured" value="1" {{ old('featured', $project->featured) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-blue-600">
            ...
            <div class="mb-6">
                ...
                @if($project->image)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $project->image) }}" alt="Image actuelle" class="h-32 rounded">
                </div>
                @endif
            ...
            <div id="content-{{ $lang }}" class="{{ $lang === 'es' ? '' : 'hidden' }} lang-content">
                <div class="mb-4">
                    ...
                    <input type="text" name="title[{{ $lang }}]" value="{{ old('title.'.$lang, $project->title[$lang] ?? '') }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Description courte ({{ strtoupper($lang) }})</label>
                    <textarea name="description[{{ $lang }}]" rows="5" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('description.'.$lang, $project->description[$lang] ?? '') }}</textarea>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Contenu ({{ strtoupper($lang) }})</label>
                    <textarea name="content[{{ $lang }}]" rows="15" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('content.'.$lang, $project->content[$lang] ?? '') }}</textarea>
                </div>
            </div>
            @endforeach
