    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-800">Modifier : {{ $member->name['es'] ?? 'Membre' }}</h2>
            </div>
            
            <form action="{{ route('admin.team.update', $member) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $member->email) }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>

                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Téléphone</label>
                        <input type="text" name="phone" value="{{ old('phone', $member->phone) }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>

                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Ordre</label>
                        <input type="number" name="order" value="{{ old('order', $member->order) }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="category" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                            ...
                            @foreach(App\Models\TeamMember::CATEGORIES as $key => $label)
                                <option value="{{ $key }}" {{ old('category', $member->category) === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Statut</label>
                         <label class="flex items-center mt-2">
                            <input type="checkbox" name="active" value="1" {{ old('active', $member->active) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-teal-600">
                ...
                <div class="mb-6">
                    ...
                    @if($member->photo)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $member->photo) }}" alt="Photo actuelle" class="h-32 rounded-full object-cover">
                        </div>
                    @endif
                ...
                <div id="content-{{ $lang }}" class="{{ $lang === 'es' ? '' : 'hidden' }} lang-content">
                    <div class="mb-4">
                        ...
                        <input type="text" name="name[{{ $lang }}]" value="{{ old('name.'.$lang, $member->name[$lang] ?? '') }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>
                    
                    <div class="mb-4">
                        <label class="block text-gray-700 font-bold mb-2">Poste ({{ strtoupper($lang) }})</label>
                        <input type="text" name="position[{{ $lang }}]" value="{{ old('position.'.$lang, $member->position[$lang] ?? '') }}" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700 font-bold mb-2">Biographie ({{ strtoupper($lang) }})</label>
                        <textarea name="bio[{{ $lang }}]" rows="5" class="w-full px-3 py-2 border rounded border-gray-300 focus:outline-none focus:border-teal-500">{{ old('bio.'.$lang, $member->bio[$lang] ?? '') }}</textarea>
                    </div>
                </div>
                @endforeach
